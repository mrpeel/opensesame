...
var passOff, temporaryPhraseStore, passwordType, fullName,
  passChangeRequiredCount, lastPassPhraseLength, lastPassGenTimeStamp,
  successPrefix, passPhraseState;
...
window.addEventListener("load", function() {
  ...
  // Add input events which check if a previous required field has been skipped, and ensure that the
  //  generated password card has been cleared and hidden
  ...
  givenName.addEventListener("focusout", sendValsToExt, false);
  givenName.addEventListener("blur", sendValsToExt, false);
  ...
  familyName.addEventListener("focusout", sendValsToExt, false);
  familyName.addEventListener("blur", sendValsToExt, false);
  ...
  /* Enable UI elements */
  givenName.disabled = familyName.disabled = passPhrase.disabled =
    domainName.disabled = userName.disabled = type.disabled = false;
...
function passPhraseUpdated() {
  ...
  if (passPhrase.value.length > 0 && fullName.length > 0) {
...
function isReadyToGenerate() {
  ...
  //Trim of any www prefix, eg 'www.'  , 'www1.', 'www-87.'
  var calculatedDomainName = domainName.value.replace(/^www[\w-]*./g, "").trim()
    .toLowerCase();
  ...
  if ((givenName.value.trim().length > 0 || familyName.value.trim().length > 0) &&
    passPhrase.value.trim().length > 0 && calculatedDomainName.length > 0 &&
...
function togglePassPhraseView() {
  ...
  if (passPhrase.type === "password") {
    ...
    passPhraseDisplayButton.innerHTML =
